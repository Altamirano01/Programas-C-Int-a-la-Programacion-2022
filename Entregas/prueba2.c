  ...
  do{
    ...
    switch  (menu)
    {
      ...
      case (5):
        ...
        printf("La posicion del registro es: %d", posicion);
        if(posicion == -1){
...
void Precipitaciones(TData q){
  int i, j;
   i = 0;
   j = 0;
   printf("Las 10 mayores precipitaciones son: \n");
   while(j < 10){
    if(q.a[i].borrado == 0){
      j++;
      printf("Fecha: %ld \n", q.a[i].ddmmyyyy);
...
void OrdenarArreglo (TData *arreglo){
  ...
  for (i = 1; i < arreglo->cant; i++){
    if(arreglo->a[i].borrado == 0){
      aux = arreglo->a[i].FF;
      j = i - 1;
      while (j >= 0 && arreglo->a[j].FF < aux){
        arreglo->a[j + 1].FF = arreglo->a[j].FF;
        j = j - 1;
      }
      arreglo->a[j + 1].FF = aux;
    }
  }
}
